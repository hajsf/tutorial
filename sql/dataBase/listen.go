...
func doWork(db *sql.DB, work string) {
	// work here
	fmt.Println(work)
}
...
func getWork(db *sql.DB) {
	for {
		...
		err := db.QueryRow("SELECT messages_notify_trigger()").Scan(&work)
		if err != nil {
			fmt.Println("call to messages_notify_trigger() failed: ", err)
		...
		fmt.Println("starting work on ", work.String)
		go doWork(db, work.String)
	}
}
